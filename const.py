...
CYPER_SUBJECT_TEMPLATE = {
    ...
    "node_property": "MATCH (node {name: '%s'}) RETURN node.%s as %s",
    # 根据name查询节点，再查询与其关系为COMMON_SAMEAS（等同关系）的节点，返回等同节点的指定属性值
    "equal_node_property": "MATCH (node {name: '%s'})-[r: COMMON_SAMEAS]-(equal_node) RETURN equal_node.%s as %s",
    # 根据name查询节点，再查询特定关系的关联节点，返回关联节点的指定属性值
    "node_relation_property": "MATCH (node_a {name: '%s'})-[r: %s]->(node_b) RETURN node_b.%s as %s",
    # 根据name查询节点，返回节点的属性和关联节点
    "node_info": "MATCH (node {name: '%s'})-[relations]->(relation_nodes) RETURN node, relations, relation_nodes",
}

# 宾语查询模板
CYPER_OBJECT_TEMPLATE = {
    "node_name": "MATCH (node {%s: '%s'}) RETURN node.name as %s",
    # 根据name查询节点，再查询特定关系的关联节点，返回关联节点的指定属性值
    "node_relation_property": "MATCH (node_a)-[r: %s]->(node_b {name: '%s'}) RETURN node_a.%s as %s",
}
