...
class SolrAPIHandler(BaseLogger):
    ...
    def _escape_str(self, query_str):
        ...
        for escape_key in list(self.escape_pattern.keys()):
...
if __name__ == "__main__":
    ...
    _docs = solr_api.search_with_seg("胚胎是如何形成的", query_fields=_query_fields, extend_condition="+(attribute_name: name)")
    for doc in _docs:
        print('#' * 100)
        for key in list(doc.keys()):
            print(key, doc[key])

    _docs = solr_api.search_index(query_str="*:*")
    for doc in _docs:
        print(doc)
